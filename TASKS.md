...
|---|---|---|---|---|---|
| 251 | 已完成 | AI-A | **群聊设置对齐聊天设置**（用户反馈：群聊右上角没有聊天设置里的全部功能）：群聊设置面板新增「输入与消息」5 开关（回车键发送 cs-enter-send on/off 语义+群聊 keydown 接线、批量发送 cs-batch-send、我可发送语音 cs-voice-send、隐藏联系人的表情包 hide-ta-sticker 根命名空间、允许删除成员消息 cs-del-ta-msg）+「数据」组（导出流式 JSON/导入兼容三结构/清空当前群记录，仅作用当前群）+气泡操作菜单「删除」按钮（ma-del-gc，按 cs-del-ta-msg 显隐、仅成员消息真删除） | src/js/group-chat.js, src/css/group-chat.css, src/template.html, build.mjs, tools/verify-gc-settings.mjs | 已完成（v3.26.x，sw mochi-mtrcpn5w+）：verify-gc-settings 扩到 34/34（S9-S12/R17-R20）；哨兵+2=540 哑 0；群聊家族复跑仅存量红（gc-more 3/gc-pool-scope 1/gc-send 1=stash 对照 HEAD 同样红）；布局 10/10；group-chat-fixes 19/19 |
| 133 | 待认领 | | **idb.js 冷启动早期读可靠性核查**（#245 排查副产品，数据层/AI-B 域）：冷启动早期窗口内 `idbGet('…:chat-msgs')` 对**已存在**的大键 resolve undefined、`idbHasKey` 返回 false（无头复现：种键后 reload 即读，多次必现）——疑似与 v3.28.x 大键挂起（__xyIdbDeferredKeys 预算）或回填竞态有关。后果链：chat.js #90「确认空库」分支被误入 → LS 会话快照回写 IDB 顶掉历史 + 权威收尾缺历史（#245 已在 chat.js 侧加账本矛盾守卫兜底，但探测层说谎的根因在 idb.js）。请核查：①挂起/预算期内 idbGet/HasKey 应返回「未知」而非「确认无」；②是否有比 5s scheduleIdbRetry 更快的 hydrate 路径 | src/js/idb.js, src/js/chat.js | 证据与复现：tools/verify-chat-lite-upgrade.mjs（红绿对照见 WORKLOG 2026-09-07 08:0x #245 条）；#245 修复时该分支已加账本守卫，不与本任务冲突 |
